...
from marcodoc.dataset import MSMARCODataset, get_collate_function
...
def train(args, model):
    ...
    train_dataset = MSMARCODataset("train", args.msmarco_dir, 
            args.collection_memmap_dir, args.tokenize_dir, args.bert_tokenizer_path,
        ...
        for step, (batch, _, _) in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.evaluate_during_training and (global_step % args.training_eval_steps == 0):
                    ...
                    tb_writer.add_scalar('ndcg20', mrr, global_step)
...
def trec_eval(runf):
    qrelf = '/data/users/liuyuan/clueweb09/top200/top200tokenized/qrel'
    trec_eval_f = '/data/users/liuyuan/bert-reranker/eval/trec_eval'
    output = subprocess.check_output([trec_eval_f, '-m', 'ndcg_cut.20', qrelf, runf]).decode().rstrip()
...
def run_parse_args():
    ...
    parser.add_argument("--msmarco_dir", type=str, default=f"./msmarco_document/data/msmarco-doc")
    parser.add_argument("--collection_memmap_dir", type=str, default="/data/users/liuyuan/PRADA_trigger/msmarco_document/tokenized_collection_memmap")
    parser.add_argument("--tokenize_dir", type=str, default="/data/users/liuyuan/PRADA_trigger/msmarco_document/tokenize_dir")
    ...
    parser.add_argument("--training_eval_steps", type=int, default=500)
    ...
    parser.add_argument("--bert_tokenizer_path", default='/data/users/liuyuan/RankGPT/deberta-10k-rank_net', type=str)
    ...
    args.eval_save_dir = f"{args.output_dir}/tmp_eval_results"
    return args
...
def main():
    ...
    if args.task == "train":
        load_model_path = f"/data/users/liuyuan/bert-base-uncased"
    else:
        assert args.eval_ckpt is not None
        load_model_path = '/data/users/liuyuan/RankGPT/deberta-10k-rank_net'
        # load_model_path = '/data/users/liuyuan/PRADA/data/train/models/ckpt-8000'
        # load_model_path = f"{args.model_save_dir}/ckpt-{args.eval_ckpt}"

    config = AutoConfig.from_pretrained('/data/users/liuyuan/RankGPT/deberta-10k-rank_net')
    model = RankingBERT_Train.from_pretrained(load_model_path, config=config)
