...
def load_reference_from_stream(f, dataset):
...
def load_reference(path_to_reference, dataset):
    """Load Reference reference relevant passages
    Args:path_to_reference (str): path to a file to load.
    Returns:qids_to_relevant_passageids (dict): dictionary mapping from query_id (int) to relevant passages (list of ints). 
    """
    with open(path_to_reference,'r') as f:
        qids_to_relevant_passageids = load_reference_from_stream(f, dataset)
    return qids_to_relevant_passageids
...
def compute_metrics_from_files(path_to_reference, path_to_candidate, dataset, perform_checks=True):
    """Compute MRR metric
    Args:    
    ...
    qids_to_relevant_passageids = load_reference(path_to_reference, dataset)
...
def main():
    ...
    dataset = sys.argv[3]
    metrics = compute_metrics_from_files(path_to_reference, path_to_candidate, dataset)
    print('#####################')
